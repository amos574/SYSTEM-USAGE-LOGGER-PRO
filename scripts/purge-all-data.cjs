/**
 * Complete Database Purge & User Reset Script for SYSTEM USAGE LOGGER PRO
 * 
 * Safely purges ALL documents from Firestore collections while preserving
 * collection names, schemas, security rules, and user profiles (with reset fields).
 */
...
async function purgeCollectionDirect(collectionName) {
  ...
  try {
    ...
    if (totalDocs === 0) {
      console.log(`[CLEAN] '${collectionName}' is already empty.`);
      return 0;
    }
...
async function main() {
  console.log(`****************************************************`);
  console.log(` SYSTEM USAGE LOGGER PRO — COMPLETE DATA WIPE       `);
  console.log(`****************************************************`);
  console.log(`Project ID: ${firebaseConfig.projectId}`);

  // 1. First attempt via server-side purge endpoint if server is up
  ...
  console.log(`\n[COMPLETED] All collections wiped clean. Schemas and rules preserved.`);
  process.exit(0);
}

main().catch((err) => {
  console.error('[FATAL] Purge script failed:', err);
  process.exit(1);
});
